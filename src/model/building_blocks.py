...
# from src.model.transformer import DualTransformer, TransformerDecoder
...
class AttentivePooling(nn.Module):
    ...
    def forward(self, feats, f_masks=None):
        ...
        # attw_norm = alpha.transpose(1,2) / torch.max(alpha.transpose(1,2), dim=-1)[0].unsqueeze(-1) # [B,att_n,A]
        attw =  F.softmax(alpha.transpose(1,2), dim=2) # [B,att_n,A]
        attw_norm = attw / torch.max(attw, dim=-1)[0].unsqueeze(-1) # [B,att_n,A]

        att_feats = attw @ feats # [B,att_n,D]
        if self.att_n == 1:
            att_feats = att_feats.squeeze(1)
            attw = attw.squeeze(1)
            attw_norm = attw_norm.squeeze(1)
        if self.use_embedding: att_feats = self.fc(att_feats)

        return att_feats, attw, attw_norm

class MultiAttentionPooling(nn.Module):
    def __init__(self, config, prefix=""):
        super(MultiAttentionPooling, self).__init__()
        name = prefix if prefix is "" else prefix+"_"
        print("Multi Attention Pooling - ", name)

        self.att_n = config.get(name+"att_n", 1)
        self.num_proposals = config.get("num_proposals", 1)
        self.feat_dim = config.get(name+"att_cand_dim", -1)
        self.att_hid_dim = config.get(name+"att_hdim", -1)
        self.att_video_dim = config.get(name+"att_vdim", 512)
        self.use_embedding = config.get(name+"att_use_embedding", True)
        self.use_guassian = config.get("use_guassian_multi_att", False)
        self.sigma = config.get("sigma", 9)

        self.feat2att = nn.Linear(self.feat_dim, self.att_hid_dim, bias=False)
        self.to_multi_head = nn.Linear(self.att_hid_dim, self.num_proposals, bias=False)
        self.fc_gauss = nn.Linear(self.att_video_dim, self.num_proposals*2, bias=False)
    ...
    def forward(self, feats, f_masks=None):
        ...
        # compute attention weights
        dot = torch.tanh(attn_f)        # [B,A,hdim]
        alpha = self.to_multi_head(dot)      # [B,A,att_n]
        if f_masks is not None:
            alpha = alpha.masked_fill(f_masks.float().unsqueeze(2).eq(0), -1e9)
        # attw_norm = alpha.transpose(1,2) / torch.max(alpha.transpose(1,2), dim=-1)[0].unsqueeze(-1) # [B,att_n,A]
        attw =  F.softmax(alpha.transpose(1,2), dim=2) # [B,att_n,A]
...
class LocalGlobalVideoTextInteractions(nn.Module):
    ...
    def _global_context_modeling(self, ss_feats, se_feats, seg_masks):
        ...
        _, sattw, _ = self.satt_fn(se_feats)
...
class AttentionLocRegressor(nn.Module):
    def __init__(self, config, prefix=""):
        ...
        self.num_proposals = config.get("num_proposals", 8)
        if self.num_proposals > 1:
            self.tatt = MultiAttentionPooling(config, "grounding")
        else:
            self.tatt = AttentivePooling(config, "grounding")
        self.fc1 = nn.Linear(idim, gdim)
        
        nn_list = [nn.Linear(idim, gdim), nn.ReLU(), nn.Linear(gdim, 2)]
        nn_list2 = [ self.fc1, nn.ReLU(), nn.Linear(gdim, 1)]
        if config["dataset"] == "charades":
            nn_list.append(nn.ReLU())
        else:
            nn_list.append(nn.Sigmoid())
        self.MLP_reg = nn.Sequential(*nn_list)
        self.iou_reg = nn.Sequential(*nn_list2)

    def forward(self, semantic_aware_seg_feats, masks):
        # perform Eq. (13) and (14)
        # [B,num_proposal,D], [B,num_proposal,A]
        summarized_vfeat, att_w, attw_norm = self.tatt(semantic_aware_seg_feats, masks)
        # perform Eq. (15)
        # [B, num_proposal, 2]
        loc = self.MLP_reg(summarized_vfeat) # loc = [t^s, t^e]

        return loc, att_w, summarized_vfeat, attw_norm
...
class TAGLoss(nn.Module):
    ...
    def forward(self, net_outs, gts):
        ...
        if torch.any(torch.isnan(ac_loss)):
            print("tag_loss", ac_loss)

        return ac_loss
    
class MultiTAGLoss(nn.Module):
    def __init__(self, cfg, prefix=""):
        super(MultiTAGLoss, self).__init__()
        self.name = prefix if prefix is "" else prefix+"_"
        print("Multi Temporal Attention Guidance Loss - ", self.name)
        self.kl_loss = nn.KLDivLoss(reduction="batchmean")

        self.w = cfg.get("multi_tag_weight", 1.0)

    def forward(self, net_outs, gts):
        attw_norm = net_outs["attw_norm"]
        multi_gauss_weight = net_outs["multi_gauss_weight"]

        attw_norm = F.log_softmax(attw_norm, dim=-1)
        multi_gauss_weight = F.softmax(multi_gauss_weight, dim=-1)

        loss = self.kl_loss(attw_norm, multi_gauss_weight)
...
class L1Loss(nn.Module):
    def __init__(self, cfg, prefix=""):
        ...
        self.name = prefix if prefix is "" else prefix+"_"
        print("L1 Loss - ", self.name)
        self.threshold = cfg.get("l1_threshold", 0.3)
        self.alpha = cfg.get("l1_alpha", 0.5)

    def forward(self, net_outs, gts):
        sim = net_outs[self.name+"cos_similarity"] #[B]
        threshold = torch.asarray([self.threshold]).expand_as(sim).to(sim.device)

        loss = F.l1_loss(sim, threshold)
...
class NCELoss(nn.Module):
    def __init__(self, cfg, prefix=""):
        ...
        self.name = prefix if prefix is "" else prefix+"_"
        print("NCE Loss - ", self.name)
        self.loss_func = nn.CrossEntropyLoss()
        self.tau = cfg.get("l1_tau", 0.07)
        self.alpha = cfg.get("nce_alpha", 1.)

    def forward(self, net_outs, gts):
        # feat1, feat2 = (bsz, n_dim)
        feat1 = net_outs[self.name+"video_feats"]
        
        feat2 = net_outs[self.name+"sen_feats"]
        
        target = torch.arange(feat1.shape[0]).cuda()
        feat1 = feat1 / feat1.norm(dim=-1, keepdim=True)
        feat2 = feat2 / feat2.norm(dim=-1, keepdim=True)
        logits = feat1 @ feat2.t() / self.tau
        loss = self.loss_func(logits, target)
        
        return self.alpha * loss
    
    def forward_twotype(self, net_outs, gts):
        feat1 = net_outs[self.name+"video_feats"]
        feat2 = net_outs[self.name+"sen_feats"]
        ...
        logits1 = feat1 @ feat2.t() / self.tau
        loss1 = self.loss_func(logits1, target)

        logits2 = feat2 @ feat1.t() / self.tau
        loss2 = self.loss_func(logits2, target)
        return ((loss1 + loss2) / 2) * self.alpha


class AttDivLoss(nn.Module):
    def __init__(self, cfg, prefix=""):
        super(AttDivLoss, self).__init__()
        self.name = prefix if prefix is "" else prefix+"_"
        print("Diversity Loss - ", self.name)
        self.num_proposals = cfg.get("num_proposals", 8)
        self.alpha = cfg.get("att_div_alpha", 1.)
        self.att_div_lambda = cfg.get("att_div_lambda", 1.)
        self.max_epoch = cfg.get("max_epoch", 500)
        self.att_div_decay = cfg.get("att_div_decay", 20)
        self.decay_amplitude = cfg.get("decay_amplitude", 0.4)
        self.min_att_div_alpha = cfg.get("min_att_div_alpha", 0.001)
    def forward(self, net_outs, gts):
        multi_t_attw = net_outs[self.name+"multi_t_attw"]
        epoch = net_outs["epoch"]
        if multi_t_attw != None:
            target = torch.eye(self.num_proposals).unsqueeze(0).cuda() * self.att_div_lambda
            source = torch.matmul(F.normalize(multi_t_attw,dim=-1, p=2), F.normalize(multi_t_attw,dim=-1, p=2).transpose(1, 2))
            # div_loss = kwargs["alpha_2"]*(torch.norm(target - source, dim=(1, 2))**2).mean()
            txt_div_loss = (torch.norm(target - source, dim=(1, 2))**2).mean()
            loss = txt_div_loss
        decay_num = epoch // self.att_div_decay
        alpha = self.alpha * (self.decay_amplitude**decay_num)
        alpha = max(alpha, self.min_att_div_alpha)
        return loss * alpha

class IntraNCELoss(nn.Module):
    def __init__(self, cfg, prefix=""):
        super(IntraNCELoss, self).__init__()
        self.name = prefix if prefix is "" else prefix+"_"
        print("Intra NCE Loss - ", self.name)
        self.loss_func = nn.CosineSimilarity()
        self.alpha = cfg.get("intra_nce_alpha", 1.)
        self.margin_1 = cfg.get("intra_margin_1", 0.25)
        self.margin_2 = cfg.get("intra_margin_2", 0.3)

    def forward(self, net_outs, gts):
        # feat1, feat2 = (bsz, n_dim)
        pos_feat = net_outs["pos_feat"] / net_outs["pos_feat"].norm(dim=-1, keepdim=True)
        neg_1_feat = net_outs["neg_1_feat"] / net_outs["neg_1_feat"].norm(dim=-1, keepdim=True)
        neg_2_feat = net_outs["neg_2_feat"]/ net_outs["neg_2_feat"].norm(dim=-1, keepdim=True)
        ref_feat = net_outs["ref_feat"] / net_outs["ref_feat"].norm(dim=-1, keepdim=True)
        sen_feats = net_outs["sen_feats"] / net_outs["sen_feats"].norm(dim=-1, keepdim=True)

        pos_cos = self.loss_func(pos_feat, sen_feats)
        neg_1_cos = self.loss_func(neg_1_feat, sen_feats)
        neg_2_cos = self.loss_func(neg_2_feat, sen_feats)
        ref_cos = self.loss_func(ref_feat, sen_feats)
        rank_loss = 0

        if ref_cos != None:
            tmp_0 = torch.zeros_like(pos_cos).cuda()
            tmp_0.requires_grad = False
            ref_loss = torch.max(ref_cos - pos_cos  + self.margin_1, tmp_0)
            rank_loss = ref_loss.mean()

        if neg_1_cos != None:
            tmp_0 = torch.zeros_like(pos_cos).cuda()
            tmp_0.requires_grad = False
            neg_1_loss = torch.max(neg_1_cos - pos_cos  + self.margin_2, tmp_0)
            rank_loss += neg_1_loss.mean()

        if neg_2_cos != None:
            tmp_0 = torch.zeros_like(pos_cos).cuda()
            tmp_0.requires_grad = False
            neg_2_loss = torch.max(neg_2_cos - pos_cos  + self.margin_2, tmp_0)
            rank_loss += neg_2_loss.mean()

        return rank_loss * self.alpha
...
class TripletLoss(nn.Module):
    def __init__(self, cfg, prefix=""):
        ...
        self.name = prefix if prefix is "" else prefix+"_"
        print("TripletLoss Loss - ", self.name)
        self.margin = cfg.get("trip_margin", 0.5)
        self.margin2 = cfg.get("trip_margin2", 0.5)
        self.margin3 = cfg.get("trip_margin3", 0.3)
        self.alpha = cfg.get("trip_alpha", 1.)
        self.cos = nn.CosineSimilarity(dim=-1, eps=1e-6)

    def forward(self, net_outs, gts):
        # video_feats=(bsz, n_frame, d) summarized_vfeat=(bsz,num_proposals, d) sen_feats=(bsz,d)
        video_feats = net_outs["sa_feats"] / net_outs["sa_feats"].norm(dim=-1, keepdim=True)
        summarized_vfeat = net_outs["summarized_vfeat"] / net_outs["summarized_vfeat"].norm(dim=-1, keepdim=True)
        sen_feats = net_outs["sen_feats"] / net_outs["sen_feats"].norm(dim=-1, keepdim=True)
        s_gt = gts["grounding_start_pos"]     # [B]
        e_gt = gts["grounding_end_pos"]       # [B]

        s_gt_idx = torch.round(s_gt * video_feats.shape[1]).clamp(0, video_feats.shape[1]-1).long()
        e_gt_idx = torch.round(e_gt * video_feats.shape[1]).clamp(0, video_feats.shape[1]-1).long()
        # gt_video_feats = (bsz, d)
        gt_video_feats = torch.stack([video_feats[i, s_gt_idx[i]:e_gt_idx[i]+1].mean(dim=0) for i in range(video_feats.shape[0])], dim=0)
        
        gt_cos_sim = torch.bmm(gt_video_feats.unsqueeze(1), sen_feats.unsqueeze(-1)).squeeze(-1).squeeze(-1)
        cos_sim = torch.bmm(summarized_vfeat, sen_feats.unsqueeze(-1)).squeeze(-1)
        neg_cos_sim = cos_sim[:,1].unsqueeze(-1)
        pos_cos_sim = cos_sim[:,0].unsqueeze(-1)
        # gt_cos_sim = self.cos(gt_video_feats, sen_feats)
        # neg_cos_sim = self.cos(summarized_vfeat.transpose(1,2), sen_feats.unsqueeze(-1))

        gt_cos_sim = gt_cos_sim.unsqueeze(-1).expand_as(neg_cos_sim)
        loss1 = torch.max(neg_cos_sim - gt_cos_sim + self.margin, torch.zeros_like(gt_cos_sim))
        loss2 = torch.max(gt_cos_sim - pos_cos_sim - self.margin2, torch.zeros_like(gt_cos_sim))
        loss3 = torch.max(pos_cos_sim - gt_cos_sim + self.margin3, torch.zeros_like(gt_cos_sim))
...
class ProposalNCELoss(nn.Module):
    def __init__(self, cfg, prefix=""):
        super(ProposalNCELoss, self).__init__()
        self.name = prefix if prefix is "" else prefix+"_"
        print("Proposal NCE Loss - ", self.name)
        self.loss_func = nn.CrossEntropyLoss()
        self.tau = cfg.get("l1_tau", 0.07)
        self.alpha = cfg.get("proposal_nce_alpha", 1.)

    def forward(self, net_outs, gts):
        # sorted_proposal_feats, sen_feats = (bsz, numproposal, n_dim)(bsz, n_dim)
        sorted_proposal_feats = net_outs[self.name+"sorted_proposal_feats"]
        sen_feats = net_outs[self.name+"sen_feats"]
        target = torch.zeros(sorted_proposal_feats.shape[0]).type(torch.LongTensor).cuda()

        sorted_proposal_feats = sorted_proposal_feats / sorted_proposal_feats.norm(dim=-1, keepdim=True)
        sen_feats = sen_feats / sen_feats.norm(dim=-1, keepdim=True)
        one_tmp = torch.ones(1).cuda()
        neg_att = (torch.arange(sorted_proposal_feats.shape[1]-1).cuda()+1) / (sorted_proposal_feats.shape[1]-1)
        att = torch.concat((one_tmp, neg_att))
        logits = torch.bmm(sorted_proposal_feats, sen_feats.unsqueeze(-1)).squeeze(-1) / self.tau
        logits = logits * att.unsqueeze(0)
        loss = self.loss_func(logits, target)
