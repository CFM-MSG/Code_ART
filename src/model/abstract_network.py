...
class AbstractNetwork(nn.Module):
    ...
    def forward_update(self, net_inps, gts):
        ...
        net_out = self.forward(net_inps, gts)
    ...
    def forward_only(self, net_inps, gts):
        """ Compute loss and network's output at once
        Args:
            net_inps: inputs for network; dict()
            gts: ground truth; dict()
    ...
    def _get_score_r5(self):
        return self.counters[self.evaluator.get_metricR5()].get_average()

    def renew_best_score(self):
        cur_score = self._get_score()
        cur_score_r5 = self._get_score_r5()
        if (self.best_score is None) or (cur_score > self.best_score):
            ...
            return True
        self.log("Iteration {}: Current score {:4f}, {:4f}".format(self.it, cur_score, cur_score_r5))
        self.log("Iteration {}: Current best score {:4f}, {:4f}".format(
                self.it, self.best_score, self.best_score_r5))
        return False
