...
class ATPEncoder(nn.Module):
    ...
    def forward(self, x_inputs: torch.tensor, x_masks):
        """
        ...
        x_encoded = self.transformer_encoder(x_encoded, src_key_padding_mask=x_masks)
        return x_encoded
...
class ATPSelectorModel(nn.Module):
    ...
    def forward(self,
        ...
        x_vis_seq = x_vis_seq.permute(1, 0, 2)    # make (L, N, D); sequence firs
        ...
        x_atp_encoded = self.atp_encoder(x_encoded, x_vis_mask)[n_text_feats:]
        
        # obtain selection scores (logits)
        x_logits = self.logits(self.dropout(x_atp_encoded))

        if x_vis_mask is not None and x_vis_mask.dtype == torch.bool:
            new_attn_mask = torch.zeros_like(x_vis_mask, dtype=torch.float)
            new_attn_mask.masked_fill_(x_vis_mask, float("-inf"))
            x_vis_mask = new_attn_mask.permute(1,0).unsqueeze(-1)
            x_logits += x_vis_mask
