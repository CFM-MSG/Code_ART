...
def _get_argument_params():
    ...
    parser.add_argument('--log', help='log name', type=str)
    parser.add_argument('--seed', default=2024, help='seed name', type=int)
...
def train(config, params):
    ...
    for epoch in range(init_step, init_step+num_step):
        ...
        if (epoch > eval_after) and (epoch % eval_every == 0):
            ...
            cmf.test(config, L["test"], net, epoch, num_case, eval_logger, mode="Valid")
